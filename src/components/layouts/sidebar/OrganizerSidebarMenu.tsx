...
export function TournamentFormAdapter() {
  ...
  const addToSelectedCountries = (countryId: string) => {
    setSelectedCountries(prev => [...prev, countryId]);
  };

  const removeFromSelectedCountries = (countryId: string) => {
    setSelectedCountries(prev => prev.filter(id => id !== countryId));
  };

  const addToSelectedProvinces = (provinceId: string) => {
    setSelectedProvinces(prev => [...prev, provinceId]);
  };

  const removeFromSelectedProvinces = (provinceId: string) => {
    setSelectedProvinces(prev => prev.filter(id => id !== provinceId));
  };

  const addToSelectedCities = (cityId: string) => {
    setSelectedCities(prev => [...prev, cityId]);
  };

  const removeFromSelectedCities = (cityId: string) => {
    setSelectedCities(prev => prev.filter(id => id !== cityId));
  };

  const addToSelectedClubs = (clubId: string) => {
    setSelectedClubs(prev => [...prev, clubId]);
  };

  const removeFromSelectedClubs = (clubId: string) => {
    setSelectedClubs(prev => prev.filter(id => id !== clubId));
  };

  // Updated handler functions that call the appropriate method directly without conditionals
  const handleCountryCheckboxChange = (countryId: string, checked: boolean) => {
    const action = checked ? addToSelectedCountries : removeFromSelectedCountries;
    action(countryId);
  };

  const handleProvinceCheckboxChange = (provinceId: string, checked: boolean) => {
    const action = checked ? addToSelectedProvinces : removeFromSelectedProvinces;
    action(provinceId);
  };

  const handleCityCheckboxChange = (cityId: string, checked: boolean) => {
    const action = checked ? addToSelectedCities : removeFromSelectedCities;
    action(cityId);
  };

  const handleClubCheckboxChange = (clubId: string, checked: boolean) => {
    const action = checked ? addToSelectedClubs : removeFromSelectedClubs;
    action(clubId);
  };
    ...
    <div className="tournament-form-container">
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-1">
          <Link href="/organizer/events/create" className="hover:bg-gray-100 p-1 rounded">
            <ChevronLeft size={20} />
          </Link>
          <p className="text-sm text-gray-500">Back to event types</p>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="basic" className="tab-content">
            <Card className="card">
              ...
              <CardContent className="card-content">
                <div className="form-group">
                  <label htmlFor="name" className="block text-sm font-medium text-slate-700 mb-1 required">Nama Tournament</label>
                  <Input
                    id="name"
                    name="name"
                ...
                <div className="form-group">
                  <label htmlFor="description" className="block text-sm font-medium text-slate-700 mb-1">Deskripsi Tournament</label>
                  <Textarea
                    id="description"
                    name="description"
                ...
                <div className="form-group">
                  <label htmlFor="tournamentType" className="block text-sm font-medium text-slate-700 mb-1 required">Jenis Tournament</label>
                  <select
                    id="tournamentType"
                    name="tournamentType"
                    value={basicInfo.tournamentType}
                    onChange={handleBasicInfoChange}
                    className="select-trigger w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                    <option value="individual">Individual (Perorangan)</option>
                    <option value="team">Team (Beregu)</option>
                    <option value="mixed">Mixed (Perorangan & Beregu)</option>
                  </select>
                </div>
                <div className="form-group">
                  <label htmlFor="eventBranding" className="block text-sm font-medium text-slate-700 mb-1">Branding (Logo/Banner)</label>
            ...
            <Card className="card mt-6">
              ...
              <CardContent className="card-content">
                <div className="form-group">
                  <label htmlFor="visibility" className="block text-sm font-medium text-slate-700 mb-1">Visibilitas</label>
                  <select
                    id="visibility"
                    name="visibility"
                  ...
                  >
                    ...
                    <option value="private">Private (Hanya via undangan/kode)</option>
                ...
                {basicInfo.visibility === "private" && (
                  <div className="form-group mt-4">
                    <label htmlFor="inviteCode" className="block text-sm font-medium text-slate-700 mb-1">Kode Undangan</label>
                    <div className="flex gap-2">
                      <Input
                        ...
                        className="form-input"
                      />
                      <Button type="button" variant="outline">Generate Kode</Button>
                    </div>
            ...
            <Card className="card mt-6">
              ...
              <CardContent className="card-content">
                <div className="form-group">
                  <label htmlFor="eventHandbook" className="block text-sm font-medium text-slate-700 mb-1">Upload Handbook</label>
                  <div className="border-2 border-dashed border-gray-200 rounded-md p-6 text-center">
                    <UploadCloud className="h-8 w-8 mx-auto text-gray-400 mb-2" />
                  ...
                  <div className="mt-4">
                    ...
                    <div className="item-list">
                      {multiPricing.map((option, idx) => (
                        <div key={`pricing-${option.category.replace(/\s+/g, '-')}-${idx}`} className="item flex items-center justify-between">
                            ...
                            .map(province => (
                              <div key={province.id} className="checkbox-group">
                                <input
                                  ...
                                  onChange={(e) => handleProvinceCheckboxChange(province.id, e.target.checked)}
                                  className="checkbox"
                                />
                          ...
                          {clubs.map(club => (
                            <div key={club.id} className="checkbox-group">
                              <input
                                ...
                                onChange={(e) => handleClubCheckboxChange(club.id, e.target.checked)}
                                className="checkbox"
                              />
              ...
              <CardContent className="card-content">
                <div className="item-list">
                  {sponsors.map((sponsor, idx) => (
                    <div key={`sponsor-${sponsor.name.replace(/\s+/g, '-')}-${sponsor.level}-${idx}`} className="item flex items-center justify-between">
                      <div>
