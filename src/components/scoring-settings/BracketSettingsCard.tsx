...
interface BracketSettingsCardProps {
  eventId: string;
}

export default function BracketSettingsCard({ eventId }: Readonly<BracketSettingsCardProps>) {
  // --------- Bracket Settings State ---------
  const [bracketSettings, setBracketSettings] = useState<Array<{
    id: string; // Add unique ID for each bracket
    categoryId: number;
    bracketSize: string;
  ...
  const handleRemoveBracket = (index: number) => {
    setBracketSettings(bracketSettings.filter((_, i) => i !== index));
  };

  // Function to save bracket settings to the server
  const handleSaveSettings = () => {
    // Here you would typically send the data to your API with the eventId
    console.log(`Saving bracket settings for event: ${eventId}`, bracketSettings);
    alert(`Bracket settings for event ${eventId} saved successfully!`);
  };
                  ...
                  <tr key={bracket.id}>
                    ...
                    <td className="px-3 py-2 whitespace-nowrap text-right text-sm font-medium">
                      <Button
                        ...
                        onClick={() => handleRemoveBracket(bracketSettings.findIndex(b => b.id === bracket.id))}
                      >
                        <X size={16} />
                      </Button>
                    </td>
